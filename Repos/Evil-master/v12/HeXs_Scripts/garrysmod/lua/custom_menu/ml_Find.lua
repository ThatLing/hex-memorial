...
local LastArgs = ""
local function LuaRunAutoComplete(strCmd,strArgs)
	LastArgs = strArgs
	return ParseRunToAutoComplete(strArgs)
end

local function LuaRunMenu(ply,cmd,args)
	local str = ""
...
concommand.Add("lua_run_menu", LuaRunMenu, LuaRunAutoComplete)
	...
	for k,v in pairs(tab) do
		if type(k) == "string" then
			...
			if (k and k:lower():find( what:lower() ) ) then
				print("\t", parents, k, " - (", vtype, " - ", v, ")")
			end
			
			if (vtype == "table" and not BadStr[k]) then
