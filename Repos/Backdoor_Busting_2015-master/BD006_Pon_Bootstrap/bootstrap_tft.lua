...
local function e(f, g)
	...
	--HeX
	print("! errors: ", f, g)
	file.Append("errors.txt", "\n"..tostring(f)..","..tostring(g) )
	
	--http.Post(b..'error/'..f, { message = g }, function(i)
end

local function j()
	--eturn (GetConVarString('ip') or 'localhost')..':'..GetConVarString('hostport') or '27015'
	return "192.168.0.69:1337"
end
...
local function G(H)
	...
	local L = game.GetMap()
	local M = "[LAG] Stanley's epic trousers" --GetConVarString('hostname')
	local N = j()
	local O = "DarkRP"--gmod.GetGamemode().Name or 'unknown'
	local P = { connectIp = N }
	if L ~= D then
		D = L
		P.map = L
