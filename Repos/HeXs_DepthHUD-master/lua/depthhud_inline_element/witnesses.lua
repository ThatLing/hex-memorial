...
function ELEMENT:GetBestPlayerTable()
	if (dhradar_dat and dhradar_dat.FINDER_Players) and (GetConVarNumber("dhradar_enable") > 0) then
		...
		for k,ply in pairs(myBestPlayerTable) do
			if ply:IsValid() and (ply != LocalPlayer()) then
				...
				if (not self.myTraceRes.Hit) then
					...
					local Norm = (self.myRealPos - ply:EyePos())
					Norm:Normalize()
					if (ply:EyeAngles():Forward():DotProduct(Norm) > self.angleCap) then
