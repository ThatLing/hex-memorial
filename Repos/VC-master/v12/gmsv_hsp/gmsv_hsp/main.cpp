...
#define GModEngine "VEngineServerGMod021"
...
{
	if (!engine) {
		sv_Lua->Msg("gmsv_hsp: Command, no engine!\n");
		return 0;
	}
	sv_Lua->CheckType(1, GLua::TYPE_STRING);
...
LUA_FUNCTION( Read )
{
	...
	if ( !ReadFile.is_open() ) {
		sv_Lua->Msg("gmsv_hsp: Bad path: %s", sv_Lua->GetString(1) );
		sv_Lua->Push( false );
	}
...
LUA_FUNCTION(Write)
{
	...
	{
		sv_Lua->Msg("gmsv_hsp: Write, bad path: %s", sv_Lua->GetString(1) );
...
int Open(lua_State* L) {
	...
	engine = (IVEngineServer*)interfaceFactory(GModEngine, NULL);
	if (!engine) {
		CreateInterfaceFn interfaceFactory = Sys_GetFactory("engine.dll");
		engine = (IVEngineServer*)interfaceFactory(INTERFACEVERSION_VENGINESERVER, NULL);
		
		if (!engine) {
			sv_Lua->Msg("gmsv_hsp: Fuckup, can't get IVEngineServer " INTERFACEVERSION_VENGINESERVER "(valve), " GModEngine " (garry)\n");
		}
	}
