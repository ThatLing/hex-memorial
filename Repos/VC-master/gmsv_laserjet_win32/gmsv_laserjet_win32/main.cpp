...
LUA_FUNCTION(WriteString) {
	...
	if (strcmp(Printer, "0.0.0.0") == 0) {
		printf("[gm_laserjet]: You didn't set the IP!\n");
		sv_Lua->Push(false);
		return 1;
	}
	...
	if (WSAStartup(0x202, &wsaData) == SOCKET_ERROR) {
		printf("[gm_laserjet]: WSAStartup failed with error %d\n", WSAGetLastError() );
		WSACleanup();
		sv_Lua->Push(false);
		return 1;
	}
	...
	if ( (host = gethostbyname(Printer)) == NULL) {
		printf("[gm_laserjet]: Can't find printer @ %s, did you set the IP?\n", Printer);
		WSACleanup();
		sv_Lua->Push(false);
		return 1;
	}
	...
	if ((sockfd = socket(AF_INET,SOCK_STREAM,0)) < 0) {
		printf("[gm_laserjet]: Can't open connection to printer!\n");
		WSACleanup();
		sv_Lua->Push(false);
		return 1;
	}
	...
	if (connect(sockfd, (struct sockaddr *)&Client,sizeof(Client)) == -1) {
		printf("[gm_laserjet]: Can't connect to printer's socket!\n");
		WSACleanup();
		sv_Lua->Push(false);
		return 1;
	}
...
int Open(lua_State* L) {
	...
	printf("[gm_laserjet]: Loaded, made by HeX, code by \"Anonymous\" (hpnt.c)\n");
	return 0;
}
