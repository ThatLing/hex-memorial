...
LUA_FUNCTION(WriteByte) {
	...
	if ((LAST_BYTE > 255) || (LAST_BYTE < 0)) { //Whoops
		sv_Lua->Error("[gm_para]: WriteByte out-of-range, 0-255, got '%i'!\n", LAST_BYTE);
	}
...
int Open(lua_State *L) {
	...
	InpOut32 = LoadLibrary("garrysmod/lua/bin/InpOut32.dll");
	if (!InpOut32) {
		sv_Lua->Error("[gm_para]: garrysmod/lua/bin/InpOut32.dll is missing!!\n");
	}
