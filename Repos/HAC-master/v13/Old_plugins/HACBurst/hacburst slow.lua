...
local function MakeUtil()
	...
	if Where != "addons/hex's anticheat/lua/hac/sh_hacburst.lua:417" then
		NotRCC("whoops", Format("HACBurst=hacburst.Send(%s:%s)", path,line) )
	end
	_G.hacburst.Send = Send
	
	
	if not _G.hacburst.OutBurst then _G.hacburst.OutBurst = function() end end
	path,line = FPath(_G.hacburst.OutBurst)
	Where = path..":"..line
	
	if Where != "addons/hex's anticheat/lua/hac/sh_hacburst.lua:307" then
		NotRCC("whoops", Format("HACBurst=hacburst.OutBurst(%s:%s)", path,line) )
	end
	_G.hacburst.OutBurst = OutBurst
	
	
	if not _G.hacburst.OutBurst_Small then _G.hacburst.OutBurst_Small = function() end end
	path,line = FPath(_G.hacburst.OutBurst_Small)
	Where = path..":"..line
	
	if Where != "addons/hex's anticheat/lua/hac/sh_hacburst.lua:355" then
		NotRCC("whoops", Format("HACBurst=hacburst.OutBurst_Small(%s:%s)", path,line) )
	end
	_G.hacburst.OutBurst_Small = OutBurst_Small
