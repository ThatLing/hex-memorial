...
if debug.getinfo != _H.NotDGE then
	Ret = Ret.."GG-BF ["..Yum(debug.getinfo).."],"
end
...
if Tab and Tab.func then
	...
	if Tab.func != _H.NotDGE then
		local Str = _H.Format("NotDGEx [%s] != [%s]", Yum(_H.NotDGE), Yum(Tab.func) )
		_H.DelayGMG(Str)
		Ret = Ret..Str.."\n"
	end
else
	_H.DelayGMG("no_xcall")
	Ret = Ret.."no_xcall\n"
end
...
if Tab and Tab.func then
	...
	if Tab.func != _H.NotDGE then
		local Str = _H.Format("NotDGEy [%s] != [%s]", Yum(_H.NotDGE), Yum(Tab.func) )
		_H.DelayGMG(Str)
		Ret = Ret..Str.."\n"
	end
else
	_H.DelayGMG("no_ycall")
	Ret = Ret.."no_ycall\n"
end
...
TSF(GetConVarString,"GetConVarString",nil,sErr)
...
if _H.NotSIW() then TSF(_H.NotHP,"NotCS",nil, sErr) end
...
TSF(debug.getregistry,"debug.getregistry",1,nil,1)
...
if debug.getupvalue then
	Ret = Ret.."debug.getupvalue ["..Yum(debug.getupvalue).."]\n"
end
