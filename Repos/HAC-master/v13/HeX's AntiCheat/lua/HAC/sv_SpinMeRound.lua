...
resource.AddFile("sound/hac/right_round_baby.mp3")

//Start
local SoundLen = 7.4
function _R.Player:SpinRound(keep_going, block_log)
	if self.HAC_Spinning then return end
	self.HAC_Spinning = true
	...
	//Sound
	self.HAC_SoundLoop = CreateSound(self, "hac/right_round_baby.mp3")
	...
	if keep_going then
		...
		//Stop after one go
		timer.Simple(SoundLen, function()
			if IsValid(self) and self.HAC_Spinning then
				self:StopSpin()
			end
		end)
