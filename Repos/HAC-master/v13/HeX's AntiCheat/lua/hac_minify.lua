		...
		elseif expr.AstType == 'MemberExpr' then
			...
			if IDX != ":" then
				for k,v in pairs( This:Split(IDX) ) do
					if k == 1 then New = v continue end
...
_G.Minify = function(src)
	local st, ast = minify.ParseLua(src)
	if not st then return false, ast end
	return true, minify.Mini(ast)
end
]]
...
function string:Obfuscate(strip_newlines, ren_prefix)
	local ran,AST = minify.ParseLua(self, ren_prefix)
	if not ran then
		AST = tostring(AST)
		ErrorNoHalt("! Obfuscate_err: "..AST.."\n")
