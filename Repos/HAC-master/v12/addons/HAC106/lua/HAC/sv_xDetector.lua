...
[[
	if (debug.getupvalue(engineConsoleCommand,1) ~= nil) then
		GPP("Detour2=[engineConsoleCommand]")
	end
	XD=(XD or 0)+1
]],
...
function HAC.SendXDetector(ply)
	...
	timer.Simple(Amt * 9, function() --More than enough time to wait
		if ValidEntity(ply) then
			...
			if (ply.HACXDCheck) then
				if (ply.HACXDCheck != Amt) then
					HAC.LogAndKickFailedInit(ply, "XDCheck_Len_"..ply.HACXDCheck, HAC.LenFailMSG)
				end
			else
				HAC.LogAndKickFailedInit(ply, "XDFailure_"..ply.HACXDCheck, HAC.LenFailMSG) --If never got a reply..
			end
		end
	end)
end
...
function HAC.XDetectorInit(ply,cmd,args)
	...
	if (ply.HACXDCheck != 0) then
		HAC.LogAndKickFailedInit(ply, "XDCheck_Again_"..XD, HAC.LOLMessage)
		return
	end
