...
for k,v in pairs(file.FindInLua("includes/modules/gm_sqlite*.dll")) do --Generate default modules whitelist (in case of update)
	table.insert(HAC.CModWhiteList, Format("CMod=%s", v) )
end
...
function HAC.CModIncomming(ply,cmd,args)
	...
	if cmd == HAC.CModReportCMD then
		if table.HasValue(HAC.CModWhiteList, Args1) then
		...
		local Found,IDX,str = HAC.StringInTable(Mod:lower(), HAC.CMod_Blacklist)
		if Found then
			local What = Format("CModW=%s-%s (%s)", Mod, Size, str)
			
			if not table.HasValue(HAC.CModWhiteList, What) then
	...
	elseif cmd == HAC.CModReportCMDH then --Hidden CMod
		...
		if not table.HasValue(HAC.CModWhiteList, Args1) and not table.HasValue(ply.HACCModTable, argz) then
			table.insert(ply.HACCModTable, argz)
		end
		
	elseif cmd == HAC.MLModReportCMD then
		if table.HasValue(HAC.MLModWhiteList, Args1) then return end
		
		local Found,IDX,str = HAC.StringInTable(Mod:lower(), HAC.ENMod_Blacklist)
		if Found then
			local What = Format("MLModW=%s-%s (%s)", Mod, Size, str)
			
			if not table.HasValue(HAC.MLModWhiteList, What) then
	...
	elseif cmd == HAC.VModReportCMD then
		if table.HasValue(HAC.VModWhiteList, Args1) then return end
		
		local Found,IDX,str = HAC.StringInTable(Mod:lower(), HAC.VMod_Blacklist)
		if Found then
			local What = Format("VModW=%s-%s (%s)", Mod, Size, str)
			
			if not table.HasValue(HAC.VModWhiteList, What) then
	...
	elseif cmd == HAC.ENModReportCMD then
		...
		if table.HasValue(HAC.ENModWhiteList, Args1) then return end
		
		local Found,IDX,str = HAC.StringInTable(Mod:lower(), HAC.ENMod_Blacklist)
		if Found then
			local What = Format("ENModW=%s-%s (%s)", Mod, Size, str)
			
			if not table.HasValue(HAC.ENModWhiteList, What) then
