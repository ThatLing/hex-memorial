...
function AdvDupe.LoadDupeTableFromFile( ply, filepath )
	...
	--HeX
	ply.AdvDupe_Lastfile = dupeshare.ParsePath(filepath) or ""
	--/HeX
end
...
function AdvDupe.RecieveFileContentSave( ply, filepath )
	...
	local Dupe = FileName
	local Cont = temp
	local Path = dupeshare.ParsePath(filepath) --filepath gsub'd \\
	if (hook.Call("AdvDupeCanSave", nil, ply,Dupe,Path,Cont) != false) then
...
function AdvDupe.CreateEntityFromTable( Player, EntTable, ID, Offset, HoldAngle )
	...
	if ( EntTable.Class == "prop_physics" ) then
        ...
        // skfix: Handle old thrusters
        if ( EntTable.Class == "gmod_thruster" ) then
            if ( type( EntTable.arglist[10] ) == "boolean" ) then
                if ( EntTable.arglist[10] == true ) then
                    EntTable.arglist[10] = "PhysicsCannister.ThrusterLoop"
                else
                    EntTable.arglist[10] = ""
                end
...
function AdvDupe.CheckOkEnt( Player, EntTable )
	...
	if ( test and test.t and test.t.AdminSpawnable and !test.t.Spawnable ) then
	...
	--HeX
	local Dupe = AdvDupe.GetUndoDescription(Player) --Player.AdvDupe_Lastfile
	Dupe = Dupe:gsub("AdvDupe", "")
	Dupe = Dupe:gsub("[]([)]", "")
	Dupe = Dupe:Trim()
	
	local EGC = tostring(EntTable.Class)
	local Path = Player.AdvDupe_LastPath
	local Called = hook.Call("AdvDupeCanSpawn", nil, Player,EGC,Dupe,EntTable,Path)
	
	if (type(Called) == "string") then
		EntTable.Class = Called
		return true
		
	elseif (type(Called) == "boolean") then
		return Called
