...
umsg.PoolString "PSStart"
umsg.PoolString "PSPacket"
umsg.PoolString "PSEnd"
umsg.PoolString "PSRequestResponse"
umsg.PoolString "PSPacketReceived"
umsg.PoolString "PSConfirmEnd"
...
local error = error
local glon = glon
local iterations = CreateConVar( "gmod_poopstream_iterations", "1", { FCVAR_ARCHIVE } )
local rawget = rawget
local rawset = rawset
...
function StreamToClients( rcp, h, d, cb )
	...
	elseif rt == "Player" then
		...
		error(("Invalid type %q given to poopstream.StreamToClients for recipients!"):format(type(rcp)))
	end
...
local function CallStreamHook(pl,h,id,encdat,decdat)
	if not _hooks[h] then
		ErrorNoHalt(("PoopStreamServer: Unhandled stream %q!"):format(h))
	return end
	...
	for id,o in pairs(_outgoing) do
		...
		if o.state == 1 then -- Processing stage.
			if o.decdata then
				...
				if b then enc = err else ErrorNoHalt(("PoopStreamServer Encoding Error: %s (operation %s)"):format(err, id)) _outgoing[o.id] = nil return end
				o.encdata = string.split(enc,128)
				o.state = 2
			else
				o.state = 3
			end
			umsg.Start("PSStart",o.rf)
				umsg.Long(o.id)
...
hook.Add("Tick","PoopstreamTick",StreamTick)
...
local function DSRequest(pl,cmd,args)
	...
	if hook.Call("AcceptStream",GAMEMODE,pl,h,id) then
		...
		umsg.Start("PSRequestResponse",pl)
			umsg.Long(id)
			umsg.Bool(true)
			umsg.Long(o.id)
		umsg.End()
...
concommand.Add("__psr",DSRequest)
...
local function DSPacket(pl,cmd,args)
	...
	umsg.Start("PSPacketReceived",pl)
		umsg.Long(id)
		umsg.Long(_incoming[id].buffer:len())
	umsg.End()
end
concommand.Add("__psp",DSPacket)
...
local function DSEnd(pl, cmd, args)
	...
	if b then decdat=err else ErrorNoHalt("PoopStreamServer Decoding Error: "..err.." (operation "..id.." from "..pl:Name().."("..pl:SteamID().."))") return end
	CallStreamHook(pl,_incoming[id].handler,id,encdat,decdat)
	_incoming[id] = nil
	umsg.Start("PSConfirmEnd",pl)
		umsg.Long(id)
	umsg.End()
end
concommand.Add("__pse",DSEnd)
...
local function OnTick()
	for k,v in pairs(SharedTables) do
		for tk,tv in pairs(v) do
			...
			if not b then ErrorNoHalt("PoopstreamServer STUError: " .. err ) end
		end
	end
end
...
local function SharedTableUpdate(pl,handler,id,encoded,decoded)
	...
	if rawget(st,"__clientchanges") then
		...
		pl:PrintMessage(HUD_PRINTCONSOLE, "PoopstreamClient STError: Not allowed to change this table!")
	end
end
...
function StreamToServer(h,d,cb,acb)
	...
	RunConsoleCommand("__psr","\1",h,o.id)
	return o.id
end
...
local function CallStreamHook(h,id,encdat,decdat)
	if not _hooks[h] then
		ErrorNoHalt("PoopStreamClient: Unhandled stream "..h.."!")
	return end
	...
	for id,o in pairs(_outgoing) do
		if o.state == 1 then -- Processing stage.
			if o.decdata then
				local enc = ""
				local b,err = pcall(glon.encode, o.decdata)
				if b then enc=err else ErrorNoHalt("PoopStreamClient Encoding Error: "..err.." (operation "..o.id..")\n") _outgoing[o.id] = nil return end
			...
			return
		elseif o.state == 2 then -- Sending stage.
			RunConsoleCommand("__psp","\1",o.encdata[o.block],o.id)
			o.block = o.block+1
			if o.block > #o.encdata then o.state = 3 end
			return
		elseif o.state == 3 then -- Ending stage
			RunConsoleCommand("__pse","\1",o.id)
			o.state = 4
			return
		end
	end
...
local function DSStart(data)
	...
	if data:ReadBool() then
		...
		local decdat = ""
		local b,err = pcall(glon.decode,encdat)
		if b then decdat=err else ErrorNoHalt("PoopStreamClient Decoding Error: "..err.." (operation "..id..")\n") return end
		CallStreamHook(_incoming[id].handler,id,encdat,decdat)
		_incoming[id] = nil
	end
end
usermessage.Hook("PSStart",DSStart)
...
usermessage.Hook("PSPacket",DSPacket)
...
usermessage.Hook("PSConfirmEnd",DSConfirmEnd)
...
usermessage.Hook("PSRequestResponse",DSRequestResponse)
...
usermessage.Hook("PSPacketReceived",DSPacketReceived)
...
_R.SharedTable = {
	...
	__newindex = function(self, key, value)
		if not rawget(self,"__clientchanges") then ErrorNoHalt("PoopstreamClient STError: Not allowed to change this table!") return end
		rawget(self,"__realtable")[key] = value
		if value == nil then
			rawget(self,"__changes")[key] = "\1\2\1" -- Our delete code.
		else
...
local function OnTick()
	for k,v in pairs(SharedTables) do
		...
		if not b then ErrorNoHalt( "PoopstreamClient STUError: "..err.."\n" ) end
	end
end
hook.Add("Tick", "SharedTableTick", OnTick)
