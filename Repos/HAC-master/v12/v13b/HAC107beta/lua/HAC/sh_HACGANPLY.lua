...
if (SERVER) then	
	...
	function HACGANPLY(ply,str,typ,time,sound)
		if ValidEntity(ply) then
			umsg.Start("HAC.GANPLY", ply)
				umsg.String(str)
				umsg.Short(typ)
				umsg.Short(time)
				umsg.String(sound)
...
if (CLIENT) then
	...
	local function OldNotify(GAMEMODE,str,type,length)
		local tab = {}
	...
	usermessage.Hook("HAC.GANPLY", function(um)
		...
		OldNotify(GAMEMODE,MStr,MType,MTime)
	end)
end
