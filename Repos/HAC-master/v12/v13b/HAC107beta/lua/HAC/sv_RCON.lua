...
function HAC.BlockRCONCommands(id,request,str,ipaddr,port)
	...
	for k,v in ipairs(HAC.SERVER.RCONCommandsToBlock) do --not bad enough for a 5 min ban, just dont pass to engine
	...
	for k,v in ipairs(HAC.SERVER.RCONCommandsToBan) do --banbanban
		if (string.Left(str, string.len(v)) == v) then
			blocked = true
			break
		end
...
function HAC.LogAllRCON(ply,ipaddr,str)
	...
	file.Append("hac_rcon_log.txt", WriteLog1)
end
...
function HAC.LogFailedRCON(ipaddr,str)
	...
	file.Append("hac_bad_rcon_log.txt", WriteLog1)
