...
HAC.TotalFile		= "C:/hac_totlbans.txt"
...
HAC.Silent			= CreateConVar("hac_silent", 1, {FCVAR_REPLICATED} )
...
--- Settings ---
HAC.LenCL			= 7158		--Length of clientside
...
function HAC.DoBan(ply,cmd,args,dontban,bantime,justlog,wait)
	...
	if not ply:IsBot() then --No useless logs for bots
		HAC.WriteLog(ply, args[1], ShortMSG) --Do main log
		HAC.GetBinds(ply) --Get their config.cfg
		
		if justlog then return end --No ban!
		HAC.TotalHacks = HAC.TotalHacks + 1
		
		if HAC.StringCheckInTable(args[1], HAC.InjectionTab) and not ply.HACIsInjected then --Injected! SH etc
			ply.HACIsInjected = true
			
			HAC.InjectionKick(ply)
			hook.Call("PlayerHasCheats", nil, ply, args[1], true)
		end
	end
	...
	if ply.HACIsInjected then --Don't ban!
		...
		if not ply:IsBot() then
			hook.Call("PlayerHasCheats", nil, ply, args[1])
		end
	end
