...
function HAC.SendXDetector(ply)
	if ply and ply:IsValid() then --file.FindInLua
		...
		timer.Simple(1.1, function()
			if ply and ply:IsValid() then --require
				ply:SendLua([[
					local ret,r=pcall(function()
						require()
					end)
					local P=string.find(r,".lua:")
					if P then
						GMG("Detour1="..string.gsub(string.gsub(string.Left(r,P+3),"@",""),":","")..":RQ]")
					end
				]])
			end
		end)
		--[[
			block two, let's try this
		]]
		...
		timer.Simple(2.5, function()
			if ply and ply:IsValid() then --new require
				ply:SendLua([[
		...
		timer.Simple(2.6, function()
			if ply and ply:IsValid() then --new engineConsoleCommand
				ply:SendLua([[
					if (debug.getupvalue(engineConsoleCommand,1) ~= nil) then
						GMG("Detour2=[engineConsoleCommand]")
					end
				]])
			end
		end)
