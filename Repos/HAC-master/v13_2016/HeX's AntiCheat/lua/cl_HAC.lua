...
--[[
	...
	If you're reading this, then you have stolen my files and are a cheating dirtbag. But we already know that.
	This is only used on one server, the UnitedHosts Deathmatch. Go play somewhere else if you want to cheat.
	
	If you have any ideas on how to make this better, or something else I should add, post here:
	http://steamcommunity.com/id/MFSiNC
	
	Dear muscleman, cdriza, JamieH, camel, ZeroTheFallen:
		Stealing & leaking public code is pointless, you can download it all here:
	https://github.com/MFSiNC/HAC
]]

HAC_Installed	   = 1
HAC_Credits		   = [[
	Ideas&Testing  = Discord,C0BRA,G-Force,TGiFallen,Sykranos,Leystryku,Willox,Fangli,
					 And all those kids who think they can "hack".
...
--[[
	=== End credits ===
	Please delete this file and leave GMod forver, like the cheater that you are.
]]

local function Useless() return HAC end
local MSGHook		= "UNITEDHOSTS_HAC_UHDM_2016_MFSiNC"
local Silent		= false
...
debugoverlay		= nil
debug.debug			= ErrorNoHalt
debug.getfenv 		= ErrorNoHalt
debug.gethook		= ErrorNoHalt
debug.getlocal		= ErrorNoHalt
debug.getupvalue	= ErrorNoHalt
debug.setmetatable	= ErrorNoHalt
debug.setfenv		= ErrorNoHalt
debug.setlocal 		= ErrorNoHalt
debug.setupvalue	= ErrorNoHalt
debug.upvaluejoin	= ErrorNoHalt
debug.getmetatable	= ErrorNoHalt
jit.attach			= ErrorNoHalt
jit.on				= ErrorNoHalt
jit.off				= ErrorNoHalt
setfenv				= ErrorNoHalt
local _R			= NotDGR()
...
_R.Player.IsDormant			= error
_R.CUserCmd.SetMouseWheel 	= error
_R.CUserCmd.CommandNumber 	= error
debug.sethook()
...
local function Merge(LST,Butter,Toast)
	...
	if Toast then
		...
		if not _G[Butter] then Poop = 1 return end
...
local function EatKeys(Eat,Keys)
	...
	local function FuckKey(k,v) RCC('bind '..k..' "'..v..'; play uhdm/hac/eight.wav; alias disconnect kill; say I CHEAT BAN ME FOR"') end
	...
	FuckYou("`", 			"play uhdm/hac/still_not_working.mp3; gamemenucommand quitnoconfirm; say I POPPED IT!")
	...
	FuckYou("MOUSE2",		"+attack; sensitivity 0.1; volume 1; play uhdm/hac/highway_to_hell.mp3; say IM ON A HIGHWAY TO HELL")
...
local function IsRCC(s)
	s = NotSL(s)
	return NotSF(s,"com") or NotSF(s,"run") or NotSF(s,"exe") or NotSF(s,"cmd") or NotSF(s,"rcc") or NotSF(s,"con")
end
local function IsGlass(Hammer)
	if Poop then return true end
...
local Hammers  = ""
while true do
	local Horn = NotDGE(Bagpipes)
	if not Horn then break --[[wind()]] end
	
	Hammers = Hammers..Format("%s %s %s %s %s\n", Bagpipes, Horn.what, Horn.name, Horn.short_src, Horn.currentline)
	Bagpipes = Bagpipes + 1
end
if Bagpipes != 4 then
...
local function NotGCS(Muffin)
	local Cup = NotGCV(Muffin)
	return Cup and NotGSS(Cup) or "NoNotGCS ("..Muffin..")"
end
...
local function EatThis(k)
	if k and not Pudding[k] then
		...
		if v then
			NotBucket("\n--"..k..":\n\n"..v, k.."_EAT.lua")
			DelayBAN("EatThis="..k..(v and "" or ", NoV") )
		end
		Pudding[k] = 1
	end
end
...
NotTC(Secret(), 2, 0, function()
	if not NotIV(Cock) then
		Cock = LocalPlayer()
		return --DelayBAN("Chicken_Check=NoCock")
	end
...
local function MashPotato()
	...
	if debug and (not debug.getmetatable or debug.getmetatable != ErrorNoHalt) then
		DelayBAN("DGMT="..tostring(debug.getmetatable) )
	end
end
local function Fry()
	NotTS(10, Fry)
...
NotTC(Secret(), 160, 0, function() Fry() end)
...
HookAdd(List or "GetPlayer", function(Cont,Tonc,Cton,Tcon,Noct)
	...
	--hook.Call("GetPlayer", CME or ply, nil,nil,true,self)
...
NotHH.InitPostEntity.Hooks = function()
	...
	if not Silent then		
		...
		NotMSC(Red,		"//                               //\n")
		NotMSC(Red,		"//      ")
		NotMSC(Green,	"BURST ME BAGPIPES!")
		NotMSC(Red,		"       //\n")
		NotMSC(Red,		"//                               //\n")
		NotMSC(Red,		"///////////////////////////////////\n")
		
		NotMSC(Blue,	'\n\t"They once said it was \'a good attempt at something impossible\',"')
		NotMSC(Blue,	'\n\t"That a good AntiCheat could never be made."\n')
		NotMSC(Blue,	'\n\t"In 2014, the UnitedHosts Deathmatch closed down, and HeX quit GMod."')
		NotMSC(Blue,	'\n\t"From late 2008 right up until August 2014, he had proven them all wrong."\n')
		NotMSC(Blue,	'\n\t"HAC had successfully kept the server 100% cheater-free for over 7 years."')
		NotMSC(Blue,	'\n\t"During which time, nearly 7,400 people tried to cheat here."')
		NotMSC(Blue,	'\n\t"None of them lasted more than 15 minutes."')
		NotMSC(Blue,	'\n\t"Many marbles were lost."\n')
	...
	RunConsoleCommand("cl_unitedhosts_dm_ready", "UH_2016", 1384 or 8888, Init+8, Thong("EIGHT", 8), HACKey or 88)
