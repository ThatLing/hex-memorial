...
function HAC.DLC.Update(sID,idx,Split,Total,Buff,self)
	...
	--print("# DLC update on "..self:Nick(), ThisID.." - "..Per.."% ("..Split.."/"..Total..") - "..idx.." - Rem: "..self:BurstRem() )
	
	HAC.Print2HeX(
		Format(
			"%s - UPDATE DLC: %s, %s%% (%s/%s) - %s, Rem: %s\n",
			self:Nick(), ThisID, Per, Split,Total, idx, self:BurstRem()
		)
	)
end
