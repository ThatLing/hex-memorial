	...
	for SID64,Tab in pairs(HAC.Prof.Temp) do
		...
		if Reason then
			...
			if not Dupes[ SID ] then
				...
				HAC.Skid.Write("sk_already_got_prof.txt", SID, Nick..", "..Reason)
			end
		else
			New = New + 1
...
function HAC.Prof.Command(ply,cmd,args,str)
	...
	for k,v in pairs( Cont:Split("\n") ) do
		...
		if v:Check("http://steamcommunity.com/profiles/76561") then
			...
			if ValidString(SID) then
				...
				if Reason then
					SkipSK = SkipSK + 1
					print("! Already on SK by URL as: ", SID64, SID, Reason)
					continue
				end
			end
		end
	...
	if SkipSK > 0 then
		print("# ToScan, SkipSK skipped "..SkipSK.." from URL")
	end
...
function HAC.Prof.Stop(ply)
	if not ply:HAC_IsHeX() and HAC.Prof.Selector then return end
	print("! Aborting")
