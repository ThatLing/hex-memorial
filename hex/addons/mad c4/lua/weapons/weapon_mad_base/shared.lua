...
/*---------------------------------------------------------
  --Name: ENT:SetupDataTables() --fucking U95
...
function SWEP:ShootBulletInformation()
	...
	// Player is moving
	elseif self.Owner:KeyDown(1560) then --IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT
		// Player is aiming
		if (self:GetDTBool(1)) then
...
function SWEP:ShootEffects()
	...
	if (not self.Owner:IsNPC() and self:Clip1() < 1) then
		timer.Simple(self.Owner:GetViewModel():SequenceDuration(), function() 
			if self.Owner and self.Owner:Alive() and self:GetOwner():GetActiveWeapon():GetClass() == WeaponModel then
...
function SWEP:ShootAnimation()
	...
	if (self:Clip1() <= 0) then
		if (AllowDryFire) then
			if self:GetDTBool(3) and self.Type == 2 then
				self:SendWeaponAnim(ACT_VM_DRYFIRE_SILENCED)	// View model animation
			else
				self:SendWeaponAnim(ACT_VM_DRYFIRE) 		// View model animation
			end
		elseif self:GetDTBool(3) and self.Type == 2 then
			self:SendWeaponAnim(ACT_VM_PRIMARYATTACK_SILENCED) 	// View model animation
		else
